...
type Candidate = {
  ...
  // Height stored as text like "5' 8\""
...
function formatDate(birthdayIso: string): string {
...
function InfoPill({ title, value }: { title: string; value: string }) {
  return (
    ...
    >
      <Text style={{ color: 'white', fontSize: 12, opacity: 0.8, marginBottom: 4 }}>{title}</Text>
...
export default function CandidateDetails() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data, error } = await supabase
        .from('candidates')
        .select(
          'id, name, waist_number, gender, university_id, height_cm, birthday, hobby, image_url'
        )
        .eq('id', id as string)
        .limit(1)
        .single();
      ...
      if (error) {
        ...
        const { data: categoriesData, error: categoriesError } = await supabase
          ...
          .eq('university_id', (data as Candidate).university_id)
          .ilike('gender', (data as Candidate).gender)
          .eq('is_active', true);
  ...
  function getCategoryDisplayName(type: string): string {
    switch (type) {
      case 'king':
        return candidate?.gender.toLowerCase() === 'male' ? 'King' : 'Queen';
      case 'style':
        return 'Style';
      case 'popular':
        return 'Popular';
  ...
  async function findNeighbor(direction: 'next' | 'prev') {
    ...
    query = isNext
      ? query.gt('waist_number', candidate.waist_number)
      : query.lt('waist_number', candidate.waist_number);

    const { data, error } = await query.order('waist_number', { ascending: isNext }).limit(1);
  ...
  }, [candidate?.id, candidate?.waist_number, candidate?.university_id, candidate?.gender]);
  ...
  async function goPrev() {
    if (!prevNeighborId) return;
    router.push(`/candidate/${prevNeighborId}`);
  }

  async function goNext() {
    if (!nextNeighborId) return;
        ...
        <SafeAreaView style={{ flex: 1 }}>
          <ScrollView
            contentContainerStyle={{
              ...
              // Increase padding so content stays clear of fixed Vote bar + FloatingActions
          ...
          >
            <View style={{ flex: 1, padding: 20, backgroundColor: 'transparent' }}>
              ...
              >
                <InfoPill title="Height" value={candidate.height_cm || '—'} />
                <InfoPill
                  title="Birthday"
                  value={candidate.birthday ? formatDate(candidate.birthday) : '—'}
                />
                <InfoPill
                  title="Age"
                  value={candidate.birthday ? `${computeAge(candidate.birthday)}` : '—'}
                />
          ...
          {/* Fixed bottom controls (outside ScrollView) */}
          <View
            style={{
              position: 'absolute',
              left: '6%',
          ...
          >
            ...
            >
              ...
              >
                <Text style={{ color: 'white', fontSize: 18, fontWeight: '800' }}>Vote</Text>
              </Pressable>
            ...
            >
              ...
              >
                  ...
                  {categories.map((category) => (
                    <Pressable
                      ...
                      style={{
                        width: '48%',
                        backgroundColor:
                          selectedCategory === category.id ? '#e0f0ff' : '#f5f5f5',
                        paddingVertical: 14,
                        borderRadius: 16,
                        borderWidth: 2,
                        borderColor:
                          selectedCategory === category.id ? '#4f8cff' : '#f5f5f5',
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 12 }}>
                  ...
                  <Pressable
                    onPress={submitVote}
                    disabled={voting || !selectedCategory || (ticketsLeft ?? 0) <= 0}
                    style={{
                      flex: 1,
                      paddingVertical: 12,
                      backgroundColor:
                        voting || !selectedCategory || (ticketsLeft ?? 0) <= 0
                          ? '#c9d2ff'
                          : '#4f8cff',
                      borderRadius: 12,
                    }}
          ...
          {/* Floating quick actions – move to bottom-right, above Vote bar */}
          <FloatingActions
            style={{
              right: 12,
              bottom: insets.bottom + 84, // sits above the Vote bar nicely
              top: undefined,             // cancel the default mid-screen positioning
              transform: [{ translateY: 0 }],
              zIndex: 200,                // keep above other elements
            }}
          />
        </SafeAreaView>
      </LinearGradient>
